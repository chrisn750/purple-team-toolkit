# Invoke-BenignExploitSim for Windows
...
`Invoke-BenignExploitSim.ps1` generates defender-relevant telemetry across seven ATT&CK-aligned technique groups:
...
```powershell
.\Invoke-BenignExploitSim.ps1
```

Run only selected technique groups:

```powershell
.\Invoke-BenignExploitSim.ps1 -Techniques Discovery,LOLBin,Scripting
```

Run a more conservative pass:

```powershell
.\Invoke-BenignExploitSim.ps1 -SafeMode -SkipChildProcess
```

Write output to a log file and retain it:

```powershell
.\Invoke-BenignExploitSim.ps1 -LogPath .\validation.log -KeepLog
```

Enable the only remaining unverified technique:

```powershell
.\Invoke-BenignExploitSim.ps1 -IncludeUnverified
```
